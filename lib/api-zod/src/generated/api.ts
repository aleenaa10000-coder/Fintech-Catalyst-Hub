...
export const GetCurrentAuthUserResponse = zod.object({
  ...
  "isAdmin": zod.boolean().describe('True when the signed-in user\'s email is in the ADMIN_EMAILS allowlist.')
}),zod.null()])
})
...
}).describe('Reports whether at least one outbound email transport is configured. `provider` is `resend` when `RESEND_API_KEY` is set, `smtp` when all four `SMTP_\*` variables are set, or `none` when neither is configured. The `ok` flag is `true` whenever any provider is configured (the endpoint does not attempt a live send).\n'),
  "seedData": zod.object({
  "ok": zod.boolean(),
  "counts": zod.record(zod.string(), zod.number().min(healthCheckResponseSeedDataCountsMinOne)).describe('Row counts per seeded table — keys are `blogPosts`, `authors`, `services`, `testimonials`, `pricingPlans`, and `siteStats`.\n'),
  "error": zod.string().optional()
}).describe('Confirms that the demo-data seed has populated the public content tables. `ok` is `true` only when every counted table has at least one row. Useful right after a fresh GitHub import to verify `pnpm seed:auto` has run.\n'),
...
export const ListBlogPostsResponseItem = zod.object({
  ...
  "viewCount": zod.number().min(listBlogPostsResponseViewCountMin).describe('Lifetime view count, incremented by the public-facing post detail page on mount via `POST \/blog\/posts\/{slug}\/view`. Used to power the \"Most read\" sort option on the blog index.\n'),
 ...
 */
export const publishBlogPostBodySlugRegExp = new RegExp('^[a-z0-9][a-z0-9-]\*$');
...
export const PublishBlogPostBody = zod.object({
  "slug": zod.string().regex(publishBlogPostBodySlugRegExp).describe('Lowercase, hyphenated URL slug.'),
  ...
  "publishedAt": zod.coerce.date().optional().describe('When omitted, the post publishes immediately (server stamps `now()`). Pass a future ISO timestamp to schedule the post — it will be hidden from the public listings, detail page, sitemap, and RSS feeds until that moment passes, then appear automatically.\n'),
...
export const GetBlogPostResponse = zod.object({
  ...
  "lastSeoPingAt": zod.coerce.date().nullish().describe('Timestamp of the most recent successful IndexNow ping for this post (Bing\/Yandex\/Seznam\/Naver). `null` when the post has never been successfully pinged (e.g. INDEXNOW_KEY was unset or the post predates this feature). Surface in the admin posts list as an \"indexed N ago\" badge.\n'),
  "lastSeoPingStatus": zod.string().nullish().describe('Status string from the most recent IndexNow attempt, even if unsuccessful. One of `accepted`, `rejected`, `skipped_no_key`, `skipped_malformed_key`, `error`. `null` for posts that have never been pinged.\n'),
  "viewCount": zod.number().min(getBlogPostResponseViewCountMin).describe('Lifetime view count, incremented by the public-facing post detail page on mount via `POST \/blog\/posts\/{slug}\/view`. Used to power the \"Most read\" sort option on the blog index.\n'),
  "seoTitle": zod.string().nullish().describe('Optional override for the `<title>` tag on this post\'s detail page. When `null` the title falls back to the post\'s `title` field. Used by the admin to hand-tune SERP appearance.\n'),
  "seoDescription": zod.string().nullish().describe('Optional override for `<meta name=\"description\">` on this post\'s detail page. When `null` the description falls back to the post\'s `excerpt`.\n'),
  "seoOgImage": zod.string().nullish().describe('Optional override for `og:image` \/ `twitter:image` on this post\'s detail page. When `null` the social card falls back to the post\'s `coverImage`. Must be an absolute URL pointing to a hosted image (1200×630 recommended).\n'),
  "noIndex": zod.boolean().describe('When true, the public post detail page emits `<meta name=\"robots\" content=\"noindex,nofollow\">` so this post is excluded from search engines (still publicly accessible by URL). Useful for sponsored, outdated, or work-in-progress posts.\n'),
...
export const UpdateBlogPostResponse = zod.object({
  ...
  "lastSeoPingAt": zod.coerce.date().nullish().describe('Timestamp of the most recent successful IndexNow ping for this post (Bing\/Yandex\/Seznam\/Naver). `null` when the post has never been successfully pinged (e.g. INDEXNOW_KEY was unset or the post predates this feature). Surface in the admin posts list as an \"indexed N ago\" badge.\n'),
  "lastSeoPingStatus": zod.string().nullish().describe('Status string from the most recent IndexNow attempt, even if unsuccessful. One of `accepted`, `rejected`, `skipped_no_key`, `skipped_malformed_key`, `error`. `null` for posts that have never been pinged.\n'),
  "viewCount": zod.number().min(updateBlogPostResponseOneViewCountMin).describe('Lifetime view count, incremented by the public-facing post detail page on mount via `POST \/blog\/posts\/{slug}\/view`. Used to power the \"Most read\" sort option on the blog index.\n'),
  "seoTitle": zod.string().nullish().describe('Optional override for the `<title>` tag on this post\'s detail page. When `null` the title falls back to the post\'s `title` field. Used by the admin to hand-tune SERP appearance.\n'),
  "seoDescription": zod.string().nullish().describe('Optional override for `<meta name=\"description\">` on this post\'s detail page. When `null` the description falls back to the post\'s `excerpt`.\n'),
  "seoOgImage": zod.string().nullish().describe('Optional override for `og:image` \/ `twitter:image` on this post\'s detail page. When `null` the social card falls back to the post\'s `coverImage`. Must be an absolute URL pointing to a hosted image (1200×630 recommended).\n'),
  "noIndex": zod.boolean().describe('When true, the public post detail page emits `<meta name=\"robots\" content=\"noindex,nofollow\">` so this post is excluded from search engines (still publicly accessible by URL). Useful for sponsored, outdated, or work-in-progress posts.\n'),
  ...
  "indexNow": zod.object({
  "status": zod.enum(['accepted', 'rejected', 'skipped_no_key', 'skipped_malformed_key', 'error']),
  "httpStatus": zod.number().optional(),
  "message": zod.string(),
  "urlsSubmitted": zod.number().min(updateBlogPostResponseTwoSeoNotificationIndexNowUrlsSubmittedMin)
}),
  "google": zod.object({
...
export const BulkNoIndexBlogPostsBody = zod.object({
  "slugs": zod.array(zod.string().min(1)).min(1).max(bulkNoIndexBlogPostsBodySlugsMax).describe('Slugs of the posts to update.'),
  "noIndex": zod.boolean().describe('New value for the `noIndex` flag on every targeted post.'),
  "snoozeDays": zod.number().min(1).max(bulkNoIndexBlogPostsBodySnoozeDaysMax).optional().describe('Optional auto-unsnooze window in days. Only meaningful when `noIndex=true`: each affected post\'s `noindexUntil` is set to `now + snoozeDays \* 24h`, and an hourly background job will flip the post back to indexed when that timestamp passes. Ignored (and `noindexUntil` is cleared) when `noIndex=false`.\n')
})
...
export const BulkNoIndexBlogPostsResponse = zod.object({
  "updatedCount": zod.number().describe('Number of posts whose `noIndex` flag was changed.'),
  "noIndex": zod.boolean().describe('The value that was applied to every targeted post.'),
  ...
  "lastSeoPingAt": zod.coerce.date().nullish().describe('Timestamp of the most recent successful IndexNow ping for this post (Bing\/Yandex\/Seznam\/Naver). `null` when the post has never been successfully pinged (e.g. INDEXNOW_KEY was unset or the post predates this feature). Surface in the admin posts list as an \"indexed N ago\" badge.\n'),
  "lastSeoPingStatus": zod.string().nullish().describe('Status string from the most recent IndexNow attempt, even if unsuccessful. One of `accepted`, `rejected`, `skipped_no_key`, `skipped_malformed_key`, `error`. `null` for posts that have never been pinged.\n'),
  "viewCount": zod.number().min(bulkNoIndexBlogPostsResponsePostsItemViewCountMin).describe('Lifetime view count, incremented by the public-facing post detail page on mount via `POST \/blog\/posts\/{slug}\/view`. Used to power the \"Most read\" sort option on the blog index.\n'),
  "seoTitle": zod.string().nullish().describe('Optional override for the `<title>` tag on this post\'s detail page. When `null` the title falls back to the post\'s `title` field. Used by the admin to hand-tune SERP appearance.\n'),
  "seoDescription": zod.string().nullish().describe('Optional override for `<meta name=\"description\">` on this post\'s detail page. When `null` the description falls back to the post\'s `excerpt`.\n'),
  "seoOgImage": zod.string().nullish().describe('Optional override for `og:image` \/ `twitter:image` on this post\'s detail page. When `null` the social card falls back to the post\'s `coverImage`. Must be an absolute URL pointing to a hosted image (1200×630 recommended).\n'),
  "noIndex": zod.boolean().describe('When true, the public post detail page emits `<meta name=\"robots\" content=\"noindex,nofollow\">` so this post is excluded from search engines (still publicly accessible by URL). Useful for sponsored, outdated, or work-in-progress posts.\n'),
...
export const ListBulkNoIndexAuditQueryParams = zod.object({
  "limit": zod.coerce.number().min(1).max(listBulkNoIndexAuditQueryLimitMax).default(listBulkNoIndexAuditQueryLimitDefault)
})
...
export const ListBulkNoIndexAuditResponseItem = zod.object({
  ...
  "snoozeDays": zod.number().min(1).max(listBulkNoIndexAuditResponseSnoozeDaysMax).nullish(),
  "requestedSlugCount": zod.number().min(listBulkNoIndexAuditResponseRequestedSlugCountMin),
  "updatedCount": zod.number().min(listBulkNoIndexAuditResponseUpdatedCountMin),
  "totalViewsHidden": zod.number().min(listBulkNoIndexAuditResponseTotalViewsHiddenMin).describe('Sum of `viewCount` across the impacted posts at the moment\nthe batch ran. For `mode=reindex` this is the number of\nviews that became visible again.\n'),
  "posts": zod.array(zod.object({
  "slug": zod.string(),
  "title": zod.string(),
  "category": zod.string(),
  "viewCount": zod.number().min(listBulkNoIndexAuditResponsePostsItemViewCountMin),
  "featured": zod.boolean(),
  "publishedAt": zod.coerce.date(),
  "wasNoIndex": zod.boolean().describe('The post\'s `noIndex` flag \*before\* the batch ran. Lets the\naudit page show the diff (e.g. \"10 of 12 posts were already\nhidden — only 2 actually flipped\").\n')
})),
  "createdAt": zod.coerce.date()
})
export const ListBulkNoIndexAuditResponse = zod.array(ListBulkNoIndexAuditResponseItem)
...
export const GetNotificationSettingsResponse = zod.object({
  ...
  "weeklyDigestEnabled": zod.boolean().describe('When true, a background scheduler posts a weekly summary of\npublishing activity + lifetime post traffic to the Slack\nwebhook every 7 days.\n'),
  ...
  "weeklyDigestLastSentAt": zod.coerce.date().nullish().describe('ISO timestamp of the last successful weekly-digest send.\nDrives the cadence check (7-day minimum gap) and powers\nthe \"last sent N days ago\" pill in the admin UI.\n'),
  "publishNotifyEnabled": zod.boolean().optional().describe('When true, an email is sent to `publishNotifyEmail` whenever\na scheduled post transitions from queued to live.\n'),
  "publishNotifyEmail": zod.string().nullish().describe('Destination address for publish-notify emails. Only used when\n`publishNotifyEnabled` is true.\n')
}).describe('Public-facing view of the Slack notification settings — never\nincludes the webhook URL itself. `slackWebhookHint` is the last\n4 characters of the URL prefixed with `…` so the admin can\nrecognise which webhook is in use.\n')
...
export const UpdateNotificationSettingsBody = zod.object({
  "slackWebhookUrl": zod.string().nullish().describe('Full Slack incoming-webhook URL (must start with\n`https:\/\/hooks.slack.com\/services\/`). Pass `null` to clear.\n'),
  "slackEnabled": zod.boolean(),
  "weeklyDigestEnabled": zod.boolean().optional().describe('Optional — omit to leave the saved value untouched. Forced\nto `false` on the server side whenever `slackWebhookUrl` is\ncleared so we never sit in an \"enabled-but-no-URL\" state.\n'),
  "publishNotifyEnabled": zod.boolean().optional().describe('Optional — omit to leave the saved value untouched.\n'),
  "publishNotifyEmail": zod.string().nullish().describe('Optional — pass null to clear. Omit to leave untouched.\n')
})
...
export const RepingBlogPostIndexNowResponse = zod.object({
  "id": zod.number(),
  "slug": zod.string(),
  "title": zod.string(),
  "excerpt": zod.string(),
  ...
  "lastSeoPingAt": zod.coerce.date().nullish().describe('Timestamp of the most recent successful IndexNow ping for this post (Bing\/Yandex\/Seznam\/Naver). `null` when the post has never been successfully pinged (e.g. INDEXNOW_KEY was unset or the post predates this feature). Surface in the admin posts list as an \"indexed N ago\" badge.\n'),
  "lastSeoPingStatus": zod.string().nullish().describe('Status string from the most recent IndexNow attempt, even if unsuccessful. One of `accepted`, `rejected`, `skipped_no_key`, `skipped_malformed_key`, `error`. `null` for posts that have never been pinged.\n'),
  "viewCount": zod.number().min(repingBlogPostIndexNowResponseOneViewCountMin).describe('Lifetime view count, incremented by the public-facing post detail page on mount via `POST \/blog\/posts\/{slug}\/view`. Used to power the \"Most read\" sort option on the blog index.\n'),
  "seoTitle": zod.string().nullish().describe('Optional override for the `<title>` tag on this post\'s detail page. When `null` the title falls back to the post\'s `title` field. Used by the admin to hand-tune SERP appearance.\n'),
  "seoDescription": zod.string().nullish().describe('Optional override for `<meta name=\"description\">` on this post\'s detail page. When `null` the description falls back to the post\'s `excerpt`.\n'),
  "seoOgImage": zod.string().nullish().describe('Optional override for `og:image` \/ `twitter:image` on this post\'s detail page. When `null` the social card falls back to the post\'s `coverImage`. Must be an absolute URL pointing to a hosted image (1200×630 recommended).\n'),
  "noIndex": zod.boolean().describe('When true, the public post detail page emits `<meta name=\"robots\" content=\"noindex,nofollow\">` so this post is excluded from search engines (still publicly accessible by URL). Useful for sponsored, outdated, or work-in-progress posts.\n'),
  ...
  "httpStatus": zod.number().optional(),
  "message": zod.string(),
  "urlsSubmitted": zod.number().min(repingBlogPostIndexNowResponseTwoSeoNotificationIndexNowUrlsSubmittedMin)
}),
  "google": zod.object({
  "status": zod.enum(['attempted', 'error']),
  "httpStatus": zod.number().optional(),
  ...
  "urls": zod.array(zod.string()),
  "durationMs": zod.number().min(repingBlogPostIndexNowResponseTwoSeoNotificationDurationMsMin)
}).describe('Result of the immediate search-engine notification triggered when\na post is published or updated. The IndexNow ping is awaited (with\na short timeout) so the admin UI can show real success\/failure\nfeedback. The Google sitemap ping is best-effort and informational.\n')
})).describe('A `BlogPost` plus the result of the immediate search-engine\nnotification fired by the publish\/update endpoint.\n')
...
export const GetSitemapHealthResponse = zod.object({
  ...
  "url": zod.string().url(),
  "source": zod.string().describe('Where this URL came from on the most recent check. One of `static`, `blog`, `author`, `rss`, `manual`.\n'),
  "lastStatusCode": zod.number().nullish().describe('HTTP status from the most recent check. `null` when the request never completed (DNS failure, network timeout) — see `lastError` in that case.\n'),
  "lastError": zod.string().nullish(),
  "isBroken": zod.boolean().describe('True when the most recent check returned 4xx\/5xx OR the fetch threw.\n'),
  "brokenSince": zod.coerce.date().nullish(),
  "lastOkAt": zod.coerce.date().nullish(),
  "lastCheckedAt": zod.coerce.date(),
  "consecutiveFailures": zod.number().min(getSitemapHealthResponseResultsItemConsecutiveFailuresMin)
}).describe('Persistent state for one URL we monitor in the daily sitemap link\nchecker. Updated in place per URL on every run.\n')),
  "targetSiteUrl": zod.string().optional().describe('The base URL the link-checker walks (i.e. what `SITE_URL`\nresolves to on this server). Surfaced so the admin dashboard\ncan warn when a non-production environment is checking a\ndomain that isn\'t actually this server, which would produce\nfalse-positive \"broken\" reports.\n'),
  "dailyJobEnabled": zod.boolean().optional().describe('Whether the background daily link-check job is currently\nscheduled in this environment. False in dev \/ preview when\n`NODE_ENV !== \"production\"` and `SITE_URL` is not overridden.\nUI uses this to show a \"daily job is paused\" notice.\n')
}).describe('Aggregate response from `GET \/admin\/sitemap-health` and\n`POST \/admin\/sitemap-health\/run`. `generatedAt` is the time the\nunderlying check ran (or the most recent `lastCheckedAt` for the\nread-only GET).\n')
...
export const RunSitemapHealthResponse = zod.object({
  "generatedAt": zod.coerce.date().nullish(),
  ...
  "lastOkAt": zod.coerce.date().nullish(),
  "lastCheckedAt": zod.coerce.date(),
  "consecutiveFailures": zod.number().min(runSitemapHealthResponseResultsItemConsecutiveFailuresMin)
}).describe('Persistent state for one URL we monitor in the daily sitemap link\nchecker. Updated in place per URL on every run.\n')),
  "targetSiteUrl": zod.string().optional().describe('The base URL the link-checker walks (i.e. what `SITE_URL`\nresolves to on this server). Surfaced so the admin dashboard\ncan warn when a non-production environment is checking a\ndomain that isn\'t actually this server, which would produce\nfalse-positive \"broken\" reports.\n'),
  "dailyJobEnabled": zod.boolean().optional().describe('Whether the background daily link-check job is currently\nscheduled in this environment. False in dev \/ preview when\n`NODE_ENV !== \"production\"` and `SITE_URL` is not overridden.\nUI uses this to show a \"daily job is paused\" notice.\n')
}).describe('Aggregate response from `GET \/admin\/sitemap-health` and\n`POST \/admin\/sitemap-health\/run`. `generatedAt` is the time the\nunderlying check ran (or the most recent `lastCheckedAt` for the\nread-only GET).\n')
 ...
 */
export const CheckSingleSitemapUrlBody = zod.object({
  "url": zod.string().url().describe('Absolute http(s) URL to probe.')
}).describe('Body for `POST \/admin\/sitemap-health\/check-url`.')

export const CheckSingleSitemapUrlResponse = zod.object({
  "url": zod.string(),
  "statusCode": zod.number().nullable().describe('HTTP status code of the response (after following redirects),\nor null if the request failed before producing one (network\nerror, timeout, DNS failure, etc.).\n'),
  "error": zod.string().nullable().describe('Error message when `statusCode` is null.'),
  "isBroken": zod.boolean().describe('True when the probe failed (no status code) or the status is\noutside the 2xx\/3xx range.\n'),
  "checkedAt": zod.coerce.date().describe('Server-side timestamp of when the probe completed.')
}).describe('Result of an on-demand single-URL probe. Mirrors the shape of an\nindividual entry in the sitemap report but is \*not\* persisted.\n')
...
export const IncrementBlogPostViewResponse = zod.object({
  "slug": zod.string(),
...
export const ListFeaturedPostsResponseItem = zod.object({
  ...
  "lastSeoPingAt": zod.coerce.date().nullish().describe('Timestamp of the most recent successful IndexNow ping for this post (Bing\/Yandex\/Seznam\/Naver). `null` when the post has never been successfully pinged (e.g. INDEXNOW_KEY was unset or the post predates this feature). Surface in the admin posts list as an \"indexed N ago\" badge.\n'),
  "lastSeoPingStatus": zod.string().nullish().describe('Status string from the most recent IndexNow attempt, even if unsuccessful. One of `accepted`, `rejected`, `skipped_no_key`, `skipped_malformed_key`, `error`. `null` for posts that have never been pinged.\n'),
  "viewCount": zod.number().min(listFeaturedPostsResponseViewCountMin).describe('Lifetime view count, incremented by the public-facing post detail page on mount via `POST \/blog\/posts\/{slug}\/view`. Used to power the \"Most read\" sort option on the blog index.\n'),
  "seoTitle": zod.string().nullish().describe('Optional override for the `<title>` tag on this post\'s detail page. When `null` the title falls back to the post\'s `title` field. Used by the admin to hand-tune SERP appearance.\n'),
  "seoDescription": zod.string().nullish().describe('Optional override for `<meta name=\"description\">` on this post\'s detail page. When `null` the description falls back to the post\'s `excerpt`.\n'),
  "seoOgImage": zod.string().nullish().describe('Optional override for `og:image` \/ `twitter:image` on this post\'s detail page. When `null` the social card falls back to the post\'s `coverImage`. Must be an absolute URL pointing to a hosted image (1200×630 recommended).\n'),
  "noIndex": zod.boolean().describe('When true, the public post detail page emits `<meta name=\"robots\" content=\"noindex,nofollow\">` so this post is excluded from search engines (still publicly accessible by URL). Useful for sponsored, outdated, or work-in-progress posts.\n'),
...
export const SubmitContactFormBody = zod.object({
  "name": zod.string().min(submitContactFormBodyNameMin).max(submitContactFormBodyNameMax),
  ...
  "message": zod.string().min(submitContactFormBodyMessageMin).max(submitContactFormBodyMessageMax)
})
...
export const SubscribeToAuthorResponse = zod.object({
  ...
  "subscriberId": zod.number().describe('ID of the global newsletter_subscribers row.'),
  "email": zod.string(),
  "authorSlug": zod.string(),
  "authorName": zod.string(),
  "alreadySubscribed": zod.boolean().describe('True when the email was already linked to this author.'),
  "createdAt": zod.coerce.date()
})
...
export const GetNewsletterSubscribersResponse = zod.object({
  ...
  "count": zod.number()
})).describe('Per-day signup buckets for the last 90 days, oldest first, zero-filled.')
})
...
export const ListAuthorSubscriberSummaryResponseItem = zod.object({
  ...
  "authorName": zod.string(),
  "authorRole": zod.string(),
  "authorPhoto": zod.string().describe('Public path to the author\'s portrait (e.g. `\/author-photos\/<slug>.png`).'),
  "subscriberCount": zod.number().describe('Total active subscribers linked to this author.'),
  "last30DayCount": zod.number().describe('Subscribers added in the last 30 days (rolling window).'),
  "latestSubscribedAt": zod.coerce.date().nullish()
})
export const ListAuthorSubscriberSummaryResponse = zod.array(ListAuthorSubscriberSummaryResponseItem)
...
export const EmailFinancialHealthScoreReportBody = zod.object({
  "email": zod.string().email(),
  "score": zod.number().min(emailFinancialHealthScoreReportBodyScoreMin).max(emailFinancialHealthScoreReportBodyScoreMax),
  ...
  "title": zod.string().min(1).max(emailFinancialHealthScoreReportBodyTipsItemTitleMax),
  "body": zod.string().min(1).max(emailFinancialHealthScoreReportBodyTipsItemBodyMax)
})).min(1).max(emailFinancialHealthScoreReportBodyTipsMax),
  "marketingOptIn": zod.boolean().default(emailFinancialHealthScoreReportBodyMarketingOptInDefault)
})

export const EmailFinancialHealthScoreReportResponse = zod.object({
  "delivered": zod.boolean().describe('True if the report email was actually sent.'),
...
export const SubmitGuestPostBody = zod.object({
  "name": zod.string().min(submitGuestPostBodyNameMin).max(submitGuestPostBodyNameMax),
  "email": zod.string().email(),
  "website": zod.string().max(submitGuestPostBodyWebsiteMax).optional(),
  "topic": zod.string().min(submitGuestPostBodyTopicMin).max(submitGuestPostBodyTopicMax),
  "category": zod.string().max(submitGuestPostBodyCategoryMax).optional(),
  "pitch": zod.string().min(submitGuestPostBodyPitchMin).max(submitGuestPostBodyPitchMax),
...
export const ListCommissioningTopicsResponse = zod.array(ListCommissioningTopicsResponseItem)
...
export const ListAdminCommissioningTopicsResponse = zod.array(ListAdminCommissioningTopicsResponseItem)
...
export const CreateCommissioningTopicBody = zod.object({
  "title": zod.string().min(createCommissioningTopicBodyTitleMin).max(createCommissioningTopicBodyTitleMax),
  "angle": zod.string().max(createCommissioningTopicBodyAngleMax).optional(),
  "category": zod.string().max(createCommissioningTopicBodyCategoryMax).optional(),
  "priority": zod.number().min(createCommissioningTopicBodyPriorityMin).max(createCommissioningTopicBodyPriorityMax).optional(),
...
export const UpdateCommissioningTopicBody = zod.object({
  "title": zod.string().min(updateCommissioningTopicBodyTitleMin).max(updateCommissioningTopicBodyTitleMax),
  "angle": zod.string().max(updateCommissioningTopicBodyAngleMax).optional(),
  "category": zod.string().max(updateCommissioningTopicBodyCategoryMax).optional(),
  "priority": zod.number().min(updateCommissioningTopicBodyPriorityMin).max(updateCommissioningTopicBodyPriorityMax).optional(),
  "isActive": zod.boolean().optional()
})
