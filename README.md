...
4. **Canvas preview** — The main site preview (Preview pane) works automatically when you click **Run**. However, the Canvas has a dedicated artifact workflow (`artifacts/fintechpresshub: web`) that Replit auto-creates but does not auto-start. If the Canvas shows "Your app is not running", simply click the **▶ Play** button on the `artifacts/fintechpresshub: web` workflow card inside the Canvas — it starts immediately and the preview loads. This is a Replit platform behaviour and only needs to be done once after each fresh import.
