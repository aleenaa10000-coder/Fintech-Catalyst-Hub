...
router.get("/commissioning-topics", async (_req, res) => {
  try {
    ...
    logger.error({ err }, "commissioningTopics: failed to fetch public list");
    res.status(500).json({ error: "Failed to fetch commissioning topics" });
  }
});
...
router.get("/admin/commissioning-topics", requireAdmin, async (_req, res) => {
  try {
    ...
    logger.error({ err }, "commissioningTopics: failed to fetch admin list");
    res.status(500).json({ error: "Failed to fetch commissioning topics" });
  }
});
...
router.post("/admin/commissioning-topics", requireAdmin, async (req, res) => {
  ...
  try {
    ...
    logger.error({ err }, "commissioningTopics: failed to create");
    res.status(500).json({ error: "Failed to create commissioning topic" });
  }
});
...
router.patch("/admin/commissioning-topics/:id", requireAdmin, async (req, res) => {
  ...
  try {
    ...
    logger.error({ err }, "commissioningTopics: failed to update");
    res.status(500).json({ error: "Failed to update commissioning topic" });
  }
});
...
router.delete("/admin/commissioning-topics/:id", requireAdmin, async (req, res) => {
  ...
  try {
    ...
    logger.error({ err }, "commissioningTopics: failed to delete");
    res.status(500).json({ error: "Failed to delete commissioning topic" });
  }
});
