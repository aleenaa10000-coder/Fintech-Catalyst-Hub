...
router.post("/contact", formRateLimiter, async (req, res) => {
  ...
  if (notifyTo) {
    ...
    const subject = `New Lead: ${companyPart}${budgetPart}`;
    const lines = [
      `Name:             ${body.name}`,
      `Email:            ${body.email}`,
      body.company ? `Company:          ${body.company}` : null,
      body.phone ? `Phone:            ${body.phone}` : null,
      body.website ? `Website URL:      ${body.website}` : null,
      body.service ? `Primary Interest: ${body.service}` : null,
    ...
    const html = `
      <h2 style="font-family:system-ui,sans-serif;color:#0a2540">New Lead: ${escapeHtml(companyPart)}${budgetPart ? ` — ${escapeHtml(body.budget ?? "")}` : ""}</h2>
      <table cellpadding="6" style="font-family:system-ui,sans-serif;font-size:14px;border-collapse:collapse">
        ...
        ${body.website ? `<tr><td><b>Website URL</b></td><td><a href="${escapeHtml(body.website)}" style="color:#0052FF">${escapeHtml(body.website)}</a></td></tr>` : ""}
        ${body.service ? `<tr><td><b>Primary Interest</b></td><td>${escapeHtml(body.service)}</td></tr>` : ""}
