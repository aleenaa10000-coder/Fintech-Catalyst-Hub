...
## Content reports + email alerts

End-to-end reader-flag pipeline. Public form on every blog post → DB row → fire-and-forget email to the editorial inbox → triage in `/admin/moderation`.

- **Schema** — `content_reports` table (`id`, `contentType`, `contentId`, `contentTitle`, `contentUrl`, `reporterName`, `reporterEmail`, `reason` enum, `details`, `status` enum `"open" | "resolved" | "dismissed"`, `createdAt`, `resolvedAt`, `resolvedBy`).
- **Public POST `/api/reports`** — IP rate-limited via `formRateLimiter`. Validates with Zod, persists, then fires `void sendMail(...).catch(logger.error)` so a slow SMTP/Resend round-trip never blocks the public response. Recipient is `REPORT_NOTIFY_TO ?? CONTACT_NOTIFY_TO ?? SMTP_USER` so existing contact-form env wiring is reused; if none are set we log a warn and still 201 the report (it's safer to lose the alert than the report itself). `replyTo` is set when the reporter shared an email so editors can reply in-thread. Email body has both text + branded HTML with a "Triage in moderation inbox →" CTA pointed at `${getSiteUrl()}/admin/moderation`.
- **Admin `GET /api/admin/reports?status=` + `PATCH /api/admin/reports/:id`** — list + status transitions with the standard `requireAdmin` gate.
- **UI** — `ReportContentDialog` on every blog post (small "Report" button next to share controls). Reports tab in `admin-moderation.tsx` with status filter + resolve/dismiss actions. Open Reports stat tile + recent reports section on `admin-dashboard.tsx`.

## Author headshot overrides + 800×800 enforcement

Editors can replace any contributor headshot from the admin UI without touching the static `authors.ts` dataset. Originals stay in the repo as the fallback so reverting is one click.

- **Schema** — `author_photo_overrides` (`slug` PK, `photoUrl`, `updatedBy`, `updatedAt`). Tiny by design — one row per overridden author, nothing else.
- **API** — `routes/authorPhotos.ts`: public `GET /api/author-photos` returns `{ overrides: { slug: photoUrl } }` for the website to merge client-side. Admin `GET /api/admin/author-photos` returns the full rows (with audit fields). `PUT /api/admin/author-photos/:slug` upserts via `onConflictDoUpdate`. `DELETE /api/admin/author-photos/:slug` reverts to the static fallback. Slug regex-validated to stop path-shaped values from sneaking in.
- **Admin page `/admin/author-photos`** — lists every static author with a 96-px preview, name/role/slug, "Custom" badge when overridden, and an `ObjectUploader` configured with `imageMinDimensions={width:800,height:800}` so anything below spec triggers a non-blocking toast warning (matching the cover/OG warning UX in `admin-blog.tsx`). Upload presigns through `/api/uploads/request-url`, uploads to object storage, finalizes via `/api/uploads/finalize`, then PUTs the resulting object path. Includes a Search box and a "Revert to original" button per overridden author. Nav link added to the existing admin header in `admin-blog.tsx` next to "Moderation inbox".
- **Render-side merge** — `data/useAuthorPhotos.ts` exports `useAuthorPhotoOverrides()` (process-wide cached + subscriber-fanned-out, so N author cards on a page = 1 fetch) plus pure helpers `resolveAuthorPhoto(slug, fallback, map)` and `invalidateAuthorPhotoOverrides()`. Wired into `about.tsx`, `authors.tsx`, `author.tsx` (header avatar + "More from the team" cards + Person schema `image`), `blog-post.tsx` (top-of-post author chip + bottom author bio card), and `AuthorSubscribeCard.tsx`. Invalidation is called from the admin page after each upload/revert so the next render picks up the change immediately.
- **Editorial guidelines** — the 800×800 rule was already documented under "Author headshots & OG / social images" in `editorial-guidelines.tsx`; no change needed there.

## Public headshot submission queue

Lets readers and authors themselves submit a replacement headshot from the public author profile, without needing admin access. Lands in the same admin queue as the override editor — approve = override created, dismiss = forgotten.

- **Schema** — `author_photo_requests` (`id`, `slug`, `photoUrl`, `submitterName?`, `submitterEmail?`, `note?`, `status` enum `"pending" | "approved" | "dismissed"`, `createdAt`, `reviewedAt`, `reviewedBy`). Indexed on `status` and `slug` so the admin queue (`status = 'pending'`) and per-author lookups stay sub-millisecond.
- **Public POST `/api/author-photo-requests`** — `formRateLimiter`-gated, Zod-validated. Slug regex enforced server-side (`/^[a-z0-9-]{1,200}$/`) so the submission can't smuggle path-shaped values; we deliberately do **not** validate the slug exists in the static `authors.ts` (admin will catch unknown slugs in the queue and dismiss them). Fires the standard fire-and-forget editorial alert via `lib/mailer.ts` with the same `REPORT_NOTIFY_TO ?? CONTACT_NOTIFY_TO ?? SMTP_USER` recipient cascade as content reports, branded HTML preview of the submitted headshot + a "Review in admin queue →" CTA, and `replyTo` set when the submitter shared an email.
- **Admin endpoints** — `GET /api/admin/author-photo-requests?status=` (defaults to `pending`, supports `approved | dismissed | all`, returns `{ requests, pendingCount }`) and `PATCH /api/admin/author-photo-requests/:id` with `{ status: 'approved' | 'dismissed' }`. Approve atomically `INSERT … ON CONFLICT DO UPDATE`s the row in `author_photo_overrides` so the new headshot goes live in the same request.
- **Public dialog** — `AuthorHeadshotRequestDialog.tsx` mounted on `/authors/:slug` directly under the social row in the dark profile header. Uses `ObjectUploader` with `imageMinDimensions={800,800}` (same warn-don't-block UX as the admin uploader and cover/OG flows) plus optional name / email / note fields. Renders an inline preview card with a one-click clear before submission, switches to a confirmation state on success ("nothing goes live automatically" copy so submitters understand the review step).
- **Admin queue UI** — pending requests render at the top of `/admin/author-photos` as an amber-tinted card stack with the headshot preview, slug → resolved author name (looked up from the static `authors.ts`), submitter, optional note, and Approve / Dismiss buttons. Approve calls `invalidateAuthorPhotoOverrides()` so the website's per-author photo cache refreshes immediately.

## Auto-tracked legal page "Last updated" via git mtime

`vite.config.ts` exposes a tiny `gitFileMtimeIso(relPath)` helper that runs `git log -1 --format=%cI -- <relPath>` at config-eval time and injects the result via Vite's `define` as `__TERMS_LAST_UPDATED_ISO__` (typed in `src/global.d.ts`). The Terms page reads it through a `resolveLastUpdated()` helper that formats the ISO string into a "DD MMM YYYY" display string for `<LegalPageLayout lastUpdated={…}>` and a `YYYY-MM-DD` slice for the `WebPage` schema's `dateModified`. If the git command fails (shallow clones, non-git environments) or the constant is empty, the helper falls back to a hard-coded `FALLBACK_LAST_UPDATED_ISO` so the page always has a valid date. Same pattern can be applied to `/privacy` later by adding another `gitFileMtimeIso()` constant + `define` entry — the helper is intentionally generic.
