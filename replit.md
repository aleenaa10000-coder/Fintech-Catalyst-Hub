...
- **SPA caveat (known)**: the public blog post page is client-rendered, so `curl /blog/<slug>` returns the generic `index.html` shell. Googlebot executes JS and sees the per-page meta from Helmet, but social crawlers (LinkedIn, Slack, Discord) do not. True per-page social previews would require a prerender step or Express-side SSR for `/blog/*` routes — out of scope for this iteration.
